...
module.exports = function DataSource() {
  ...
  ); // A search tree used to keep the individual cell groups sorted for insertion into the parent cell group
  ...
  this.removeItems = function(items) {
    items.forEach(function(item) {
      savedItems.delete(orderMapping[item.id]);
    });
    for (var i = 0; i < listeners.length; i++) {
  ...
  this.clear = function() {
    this.removeItems(savedItems.toArray());
  };
  ...
  this.clearSilent = function() {
    ...
    order = 0; //The order the items were added in for iteration
    orderMapping = {};
  };
};
