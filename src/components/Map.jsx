    ...
    decoratingDataLink.onDataChanged = function(event) {
      ...
      for (i = 0; i < event.added.length; i++) {
        if (event.added[i].type === "ship") {
          ...
          decoratedAdditions[i].greatCircleAngles = [
            0,
            Math.PI / 3,
            -Math.PI / 3
          ];
          decoratedAdditions[i].latitudeAngles = [
            0,
            Math.PI / 6,
            Math.PI / 3,
            -Math.PI / 6,
          ...
          decoratedAdditions[i].borderStar = {
            radius1: 3,
            radius2: 6,
            points: 20,
            borderColor: "#f97306"
      ...
      for (i = 0; i < event.added.length; i++) {
        if (event.added[i].velocity) {
          ...
          clones.push({
            ...
            color: "#0343df",
            dragged: false,
            u: event.added[i].u + event.added[i].velocity.u,
            v: event.added[i].v + event.added[i].velocity.v
          });
          this.emitEvent("dataChanged", [
            { added: clones, removed: [], updated: [] }
          ]);
          //Connect that clone to the station with a dotted line
          connectingDataSource.addItems([
            {
              id: event.added[i].id + "velShaft",
              distance: 17.5,
              color: "blue",
              radius: 5,
              sourceGap: 10,
              destGap: 10,
              target: event.added[i].id + "velHead",
              source: event.added[i].id
            }
          ]);
        }
      }
